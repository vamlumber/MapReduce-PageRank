...
public class Entry {
	...
	static String constructKey(String[] values) {
		...
		for(String genre:genreArray) {
			...
			if(genre.contains(each[0]) && genre.contains(each[1])) {
				genreKey=genre;
			}
		}
		if(genreKey==null) {
	...
	public static class WordCountMapper extends Mapper<Object, Text, Text, IntWritable> {
		...
		protected void map(Object key, Text value, Mapper<Object, Text, Text, IntWritable>.Context context)
			...
			try {
				...
				if (year >= 2000) {
					StringTokenizer genre = new StringTokenizer(split[4], ",");
					while (genre.hasMoreElements()) {
						// for each genre create a key value pair<(year,genre),cout>
						String keyWord = year + "," + genre.nextElement();
						keyText.set(keyWord);
